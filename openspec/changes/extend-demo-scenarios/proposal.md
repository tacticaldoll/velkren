...
- **`@velkren/neutral-composition-fixture` is only partially reusable**: its `panelClass`/`fieldClass`/`buttonClass`/`templateFor` building blocks are reused inside the membrane scenario's inner composition (mirroring `createEditorApp`'s own internal wiring), but the nested-view anchor wiring, the keyed-list `RenderNode` builder, and the `MembraneConfig`/`defineVelkrenElement` registration glue do not exist in that fixture and are authored fresh in the demo package, adapted from the equivalent code already proven in each adapter's own test suite (`describe("native nested views")`, `describe("keyed child reconciliation")`, `test/membrane.test.ts`).
