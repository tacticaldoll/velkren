...
- [ ] 4.1 A test per adapter: mount a dialog view with an anchor, `mountChild` a child into it, capture the child's container element reference, commit the *parent* root again with a change that causes the view to rebuild, then assert: the child's container is the *same element reference* as captured before, it is now a descendant of the *new* anchor element, and the child's own identity/interaction registration still works (a simulated interaction still reaches it). Note the trigger differs by adapter and both are worth naming honestly, not glossed as symmetric: for Solid, *any* commit forces a rebuild (a view is unconditionally re-instantiated per the existing `patchNode` behavior), so a plain attribute-only re-commit suffices; for React/Vue, `registerAnchor`'s `ref` callback re-fires every render but with the *same* element in the common case (a no-op for the diff) — actually replacing the anchor's element requires the view itself to change that position's key or element type between commits, so the test must construct that specific scenario rather than a generic re-commit.
