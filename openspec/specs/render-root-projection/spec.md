...
The system SHALL define a `RendererPort` contract that a renderer adapter implements. The port SHALL receive renderer-neutral render nodes and a runtime-assigned identity token and return an opaque adapter root, and it SHALL expose commit, identity read, and removal operations. Core MUST NOT import DOM, JSX, CSS, renderer, or reactive-library types, and the port MUST be usable in Node.js.

#### Scenario: Implement the port without browser types

- **WHEN** an adapter implements the RendererPort using only renderer-neutral render nodes and identity tokens
- **THEN** the projection runtime drives it without core importing any DOM, JSX, or renderer type

#### Scenario: Reject a non-conforming renderer

- **WHEN** a projection runtime is created with a value that does not implement the required port operations
...
The system SHALL provide a framework-owned in-memory fake renderer that implements the RendererPort for tests. It SHALL build an inspectable node tree from render nodes, record the identity attribute on each root, and expose read access to the projected tree and identity without any browser global.
...
- **WHEN** the fake renderer projects, commits, repairs, and removes roots in a Node.js environment
...
The renderer port, projection runtime, RootHandle, identity, commit repair, and fake renderer MUST remain usable in Node.js without DOM, JSX, CSS, real-renderer, browser Event, or reactive-library dependencies.

#### Scenario: Execute projection core in Node.js

- **WHEN** the projection test suite runs in a Node.js environment
- **THEN** port invocation, mounting, identity assignment, commit repair, ownership rejection, and cleanup all execute without browser globals
