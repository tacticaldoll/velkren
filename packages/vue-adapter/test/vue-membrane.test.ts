...
// The same membrane composition as the Solid and React validations — proving the
// shared core runs unchanged on the Vue adapter's renderer.
const panelClass = createComponentClass("vue.panel", () => ({}));
const submitted = createEventClass("vue.submitted", {
...
function panelTemplate() {
  return createTemplateClass("vue.panel.default", {
    component: "component/vue.panel",
...
function editorMembrane(records: Records): MembraneConfig {
  return {
    async mount({
      ...
      const runtime = createRuntime({ id: `vue-${id}` });
...
describe("vue element membrane", () => {
  it("mounts, isolates, relays, and disposes through the boundary on Vue", async () => {
    const records = makeRecords();
    defineVelkrenElement("vue-editor", editorMembrane(records));

    const a = await place("vue-editor", "a", records);
    const b = await place("vue-editor", "b", records);
