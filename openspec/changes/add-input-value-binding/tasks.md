...
- [ ] 1.1 Add an `applyValueProperty(element, next)` helper in
...
- [ ] 1.2 Wire `applyAttributes` (initial mount) to use
      `applyValueProperty` for the `value` key when `"value" in element`,
      falling through to `setAttribute` for every other key
- [ ] 1.3 Wire `patchAttributes` (commit) the same way, including the
      removal branch: clear the property via `applyValueProperty(element, "")`
      when `value` is present in `oldAttributes` but absent from
      `newAttributes` and the element supports it, instead of
      `removeAttribute`

## 2. React adapter

- [ ] 2.1 Add a `CONTROLLED_VALUE_TAGS = new Set(["input", "textarea", "select"])`
      constant and the same `applyValueProperty(element, next)` helper in
      `packages/react-adapter/src/index.ts`
- [ ] 2.2 In `renderNode`, exclude the `value` attribute from `props` when
      `CONTROLLED_VALUE_TAGS.has(node.kind)`
- [ ] 2.3 In `renderNode`, when `CONTROLLED_VALUE_TAGS.has(node.kind)`, add a
...
- [ ] 3.1 No change to `packages/vue-adapter/src/index.ts` — confirmed by
      design.md's reading of `@vue/runtime-dom`'s `patchDOMProp`/`shouldSetAsProp`

## 4. Tests

- [ ] 4.1 `packages/solid-adapter/test/solid-adapter.test.ts`: a test that
...
- [ ] 4.2 Same test shape in `packages/react-adapter/test/react-adapter.test.ts`,
