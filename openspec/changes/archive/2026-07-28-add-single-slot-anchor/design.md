...
- No automatic mount/unmount coordinator driven by slot-reference changes across `resolvePlan` calls.
  ...
  remounting), and would extend adapter plumbing for the _entire_ primitive-rendering path rather than
...
for anchor names with a _mounted child_ being reconciled, and only detects staleness by the element
being **detached from the DOM** — which is exactly what happens when a view (or a primitive) is
rebuilt, but is _not_ what happens here: a patch-in-place keeps the same, still-attached element, so
...
`anchors`, which slot name (if any) is currently registered _for a given element_ via the slot
...
- Vue was first attempted the same way as React (a fresh `ref` closure per render, cleaning up its own
  slot name on the `null` call), on the expectation that Vue's ref reconciliation would likewise
  re-invoke a changed ref function against a reused element. A dedicated test (rename a sole slot
  across a patch-in-place commit with no rebuild) falsified that expectation: Vue did not re-invoke the
  `ref` at all for the reused element, so the stale name was never cleaned up — exactly how the prior
  `fix-anchor-replacement-orphaning` change caught its own stale-reference bug by a failing test, not
  by review alone. Vue instead registers via `onVnodeMounted`/`onVnodeUpdated` vnode lifecycle hooks
  (`vnode.el` gives the resolved element), which Vue guarantees to fire on every mount and every patch
  of that vnode position regardless of whether the underlying DOM element changed. These hooks are
  attached unconditionally on every primitive node (not only when that render has a sole slot), calling
  the same `registerSlotAnchor` helper Solid uses (a per-root `WeakMap<HTMLElement, string>`
  compare-and-clean, keyed by element), so a node that had a slot and just lost it still gets its stale
  `anchors` entry cleared on that same patch.
...
React's primitive path today attaches a `ref` callback _only_ for controlled-value elements
...
attaches a `ref` whenever _either_ concern applies (controlled value or a sole resolved slot), doing
...
React's own documented callback-ref semantics (a fresh closure every render is treated as a ref change,
re-invoked as old-ref(null) then new-ref(element) even when the underlying DOM node is reused) give the
`null` branch a reliable hook to clean up that closure's own slot registration. Vue's primitive path
has no existing `ref` usage at all (native value binding needs zero adapter code, per the existing
"native renderer already satisfies value-crossing" requirement) and, per the empirical finding above,
does not offer the same reliable re-invocation — so Vue registers via vnode lifecycle hooks instead of
`ref` (see below), not a plain new `ref` prop as originally assumed.
