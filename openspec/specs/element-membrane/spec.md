...
the Solid, React, and Vue adapters through a thin wrapper. An observed HTML
attribute and a declared data property both cross inward as a raw value delivered
to a host-registered handler, routed through the existing state/binding domain's
snapshot-or-reject boundary rather than a new one.
