...
1. Index the old children by key (`Map<key, {node, element}>`, reading `parent.children[i]` for each old child that *has* a key — an unkeyed old child is deliberately left unindexed).
