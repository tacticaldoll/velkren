...
- [x] 3.1 In the membrane element's constructor, define an accessor
      (`Object.defineProperty`) for each name in `config.dataProperties`,
      backed by a per-instance `#propertyValues: Map<string, unknown>`
- [x] 3.2 Add a per-instance `#propertyHandlers: Map<string, Set<handler>>`
...
- [x] 4.1 Wrap every handler invocation (attribute and property) in a
...
- [x] 4.2 Reset `#attributeHandlers`/`#propertyHandlers` to empty at the
...
- [x] 4.3 Confirm `#attributeValues`/`#propertyValues` are NOT reset across a
...
- [x] 5.1 `packages/solid-adapter/test/membrane.test.ts`: extend the editor
...
- [x] 5.2 `packages/react-adapter/test/react-membrane.test.ts`: same test,
      React renderer
- [x] 5.3 `packages/vue-adapter/test/vue-membrane.test.ts`: same test, Vue
      renderer
- [x] 5.4 In one of the three (no need to triplicate), add a test that
...
- [x] 5.5 In one of the three, add a test that a pre-mount attribute value
...
- [x] 6.1 Run `npm run build`, `npm test`, `npm run lint`,
      `npm run format:check` and confirm all four pass
- [x] 6.2 Grep for any adapter-package source change needed beyond the test
      files (expect none — `@velkren/element`'s change should be sufficient
      for all three adapters unchanged) — confirmed: `git status` shows only
      `packages/element/src/index.ts` and the three membrane test files
      changed; no adapter `src/` file touched
