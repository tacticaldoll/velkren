...
- Not attempting to make views stop rebuilding on every commit (that's `add-typed-view-props`/`fix-solid-commit-reconcile`-era territory, a much larger change with its own tradeoffs) — this change makes replacement _safe_, it doesn't try to prevent replacement from happening.
