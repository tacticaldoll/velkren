...
- [x] 3.2 `packages/vue-adapter/src/index.ts`: change `buildVNode`'s children-mapping call from `buildVNode(child, views, String(index))` to `buildVNode(child, views, child.key ?? String(index))`. No other change — Vue's own reconciler does the rest.
